...
def shortest_path(source, target):
    # Create the initial node with the source actor's ID
    start = Node(state=source, parent=None, action=None)

    # Initialize the frontier using a QueueFrontier (BFS)
    ...
    while not frontier.empty():
        # Remove the node from the frontier for exploration
        current_node = frontier.remove()

        # If the current node's state is the target actor, we found the path
    ...
    # If no path was found, return None (no connection between actors)
    return None
