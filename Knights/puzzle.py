...
knowledge0 = And(
    ...
    # If A is a knight, then A speaks the truth:
    Implication(AKnight, And(AKnight, AKnave)),
    # If A is a knave, then A tells a lie:
...
knowledge1 = And(
    ...
    # If A is a knight, both A and B must be knaves:
...
knowledge2 = And(
    ...
    And(Or(AKnight, AKnave), Not(And(AKnight, AKnave))),
    And(Or(BKnight, BKnave), Not(And(BKnight, BKnave))),
    # If A is a knight, both A and B must be the same (either both knights or both knaves):
    Implication(AKnight, Or(And(AKnight, BKnight), And(AKnave, BKnave))),
    # If A is a knave, then A tells a lie:
    Implication(AKnave, Not(Or(And(AKnight, BKnight), And(AKnave, BKnave)))),
    # If B is a knight, A and B must be different (one knight, one knave):
    Implication(BKnight, Or(And(AKnight, BKnave), And(AKnave, BKnight))),
    # If B is a knave, then B tells a lie:
